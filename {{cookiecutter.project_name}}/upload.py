...
def upload(host, zip_path):
    ...
    ssh.connect(hostname=host, username=SANDBOX_USERNAME, password=SANDBOX_PASSWORD, port=SANDBOX_PORT)
    ssh.exec_command('mkdir -p /home/sandbox/player')

    scp = SCPClient(ssh.get_transport())
    scp.put(os.path.join(SCRIPT_ROOT, zip_path), '/home/sandbox/player')

    ssh.exec_command(f'unzip /home/sandbox/player/{zip_path} -d /home/sandbox/player/{{cookiecutter.project_name}}')
    ssh.exec_command('/home/sandbox/player/{{cookiecutter.project_name}}/init.sh')
...
def main(argv):
    ...
    upload(args[0], '{{cookiecutter.project_name}}.zip')
